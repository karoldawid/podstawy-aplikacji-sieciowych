...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        SportsFacility gym1 = sportsFacilityService.createGym(
                new CreateGymRequest("FitFabric", 50.0, 200, 50, true)
        );

        SportsFacility court1 = sportsFacilityService.createTennisCourt(
                new CreateTennisCourtRequest("Korty \"Szybka Piłka\"", 80.0, 4, "CLAY", true)
        );

        SportsFacility pool1 = sportsFacilityService.createSwimmingPool(
                new CreateSwimmingPoolRequest("Pływalnia \"Fala\"", 30.0, 100, 25, 6)
        );


        User admin = userService.createAdmin(
                new CreateAdminRequest("kdawid", "Karol", "Dawid", "kdawid")
        );
        userService.activateUser(admin.getId());

        User manager1 = userService.createFacilityManager(
                new CreateFacilityManagerRequest("jkowalski", "Jan", "Kowalski", "kdawid")
        );
        userService.activateUser(manager1.getId());

        User client1 = userService.createClient(
                new CreateClientRequest("mchodulski", "Mateusz", "Chodulski", "kdawid")
        );
        userService.activateUser(client1.getId());

        User client2 = userService.createClient(
                new CreateClientRequest("anowak", "Anna", "Nowak", "kdawid")
        );
