...
class RestSfsApplicationTests {
    ...
    @Test
    void clientCannotRentForAnotherUserViaAdminEndpoint() {
        String adminJson = """
            { "login": "adminSetup", "firstName": "Admin", "lastName": "Testowy", "password": "pass" }
        """;
        given().contentType(ContentType.JSON).body(adminJson).when().post("/api/v1/admins").then().statusCode(201);

        String adminAuthJson = """
            { "login": "adminSetup", "password": "pass" }
        """;
        ...
        String facilityJson = """
            { "name": "Kort Secure", "pricePerHour": 50, "capacity": 2, "surfaceType": "CLAY", "isIndoor": false }
        """;
        ...
        String victimJson = """
            { "login": "victimUser", "firstName": "Adam", "lastName": "Ofiara", "password": "pass" }
        """;
        String victimId = given()
                .contentType(ContentType.JSON)
                .body(victimJson)
        ...
        String attackerJson = """
            { "login": "hackerUser", "firstName": "Zly", "lastName": "Hacker", "password": "pass" }
        """;
        given().contentType(ContentType.JSON).body(attackerJson).when().post("/api/v1/clients").then().statusCode(201);

        String hackerAuthJson = """
            { "login": "hackerUser", "password": "pass" }
        """;
        String hackerToken = given()
                .contentType(ContentType.JSON)
                .body(hackerAuthJson)
                .when().post("/api/v1/auth/login")
                .then().statusCode(200).extract().path("token");

        String maliciousRentalJson = String.format("""
        {
            "clientId": "%s",
            "facilityId": "%s",
            "startTime": "2099-06-01T12:00:00",
            "endTime": "2099-06-01T13:00:00"
        }
        """, victimId, facilityId);
